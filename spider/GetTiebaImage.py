...
def getImgUrlList(Number):
    ...
    while True:
        ...
        if Next:
            ...
            print('正在获得第 %s 页的所有图片地址，该页已经是最后一页' % p)
            i = soup.find_all(
                'img',
                class_="BDE_Image",
                src=re.compile(r'https?://imgs(a|rc).baidu.com/forum/*'))
            urlList.extend(i)
            break
    ...
    for i in urlList:
        try:
            ...
            with open(ImgName, 'wb') as f:
                f.write(Img.content)
            print('第 %s 张图片下载完成，共有 %s 张图片' % (t, length))
            t += 1
        except:
            print('第 %s 张图片下载错误，跳过该图片' % t)
            continue
    print('全部图片已下载完成')
